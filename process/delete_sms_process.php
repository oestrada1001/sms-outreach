...
if(!$db_connect){
    ...
    switch ($type){
        case 'null_headline_1':
            $sql = $db_connect->prepare("UPDATE clients SET subscription_message = ? WHERE email = ? and business_name = ?");
            $sql->bind_param("sss", $null_headline_1, $email, $businessName);
            
            $null_headline_1 = null;
            $email = $row['email'];
            ...
            break;
        case 'null_headline_2':
            $sql = $db_connect->prepare("UPDATE clients SET loyalty_message = ? WHERE email = ? and business_name = ?");
            $sql->bind_param("sss", $null_headline_2, $email, $businessName);
            
            $null_headline_2 = null;
            $email = $row['email'];
            $businessName = $row['business_name'];
