...
if($client_message == 'start'){
    ...
    if($finding_match == 1){
        ...
        if($clients_current_status == 'yes'){
            ...
            ?>
                <Response>
                    <Message>Your number is already confirmed and subscribed. Thank You.</Message>
                </Response>
            <?php
            exit;
        }

        $client_confirmation_sql = "UPDATE $business_name SET confirmed = 'yes' WHERE phone_number = '$client_number'";

        if(mysqli_query($db_connect, $client_confirmation_sql)){
            header('Content-Type: text/xml');
            ?>
                <Response>
                    <Message>Thank you for subscribing. Reply STOP if you would like to unsubscribe.</Message>
                </Response>
            <?php
            exit;
        }else{
