...
if(!$db_connect){
    ...
    switch ($type){
        case 'default_message':
            ...
            if($sql->execute()){
                echo "Your default message has been successfully set.";
                exit;
            }else{
                echo 606;
                exit;
        ...
        case 'visit_message':
            ...
            if($sql->execute()){
                echo "Your Visit Achievement Message has been successfully set.";
                exit;
            }else{
                echo 606;
                exit;
        ...
        case 'cta_headline_1':
            ...
            $sql = $db_connect->prepare("UPDATE clients SET subscription_message = ? WHERE email = ? AND business_name = ?");
            
            $sql->bind_param("sss", $subscription_message, $email, $business_name);
            
            $subscription_message = $outbound_message;
            $email = $row['email'];
            $business_name = $row['business_name'];
            
            if($sql->execute()){
                echo "Headline &#35;1 has been successfully set.";
                exit;
            }else{
                echo 606;
                exit;
        ...
        case 'cta_headline_2':
            ...
            $sql = $db_connect->prepare("UPDATE clients SET loyalty_message = ? WHERE email = ? AND business_name = ?");
            ...
            if($sql->execute()){
                echo "Headline &#35;2 has been successfully set.";
                exit;
            }else{
                echo 606;
                exit;
